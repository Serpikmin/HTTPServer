...
//#define ARRAY_SIZE(arr) (sizeof(arr) / sizeof((arr)[0])) // from man page fread
...
const int QUEUE_LENGTH = 4; // number of clients we will support
const int MAX_NUMBER_OF_REQUESTS = 20;
const int NUMBER_OF_SOCKETS = 5; // dont think we use this...
...
int setup_server_socket(struct sockaddr_in *self, int num_queue) {
  ...
  */
  ...
  void * read_requests(void * housekeeping_struct) {
      char buffer[30000];
      int connection_flag = 0;
      long valread;
      struct thread_params *housekeeping = (struct thread_params*) housekeeping_struct;
      int new_socket = (housekeeping)->client_soc;
      printf("dereferenced new socket in thread: %d\n", new_socket);
      int is_first_request = (housekeeping)->is_first_request;
      printf("dereferenced is first request in thread: %d\n", is_first_request);
      printf("reading inside of thread...\n");

      //printf("child thread about to go to sleep...\n");
      //sleep(5);

      // from stack overflow will link it
      fd_set set;
      ...
      timeout.tv_sec = 0;
      timeout.tv_usec = 900000; // this is in microseconds. Reference: 1,000,000 microseconds = 1 second
      rv = select(new_socket + 1, &set, NULL, NULL, &timeout);
      if (rv == -1) {
        perror("Error with select in child thread");
      }
      else if (rv == 0) { // it means timeout occured
        //printf("No new request recieved from socket %d in child thread\n", new_socket);
        //close(new_socket);
        //printf("Closed socket %d in child thread\n", new_socket);
        //connection_flag = 0;
        (housekeeping)->connection_flag = 0;
        pthread_exit(0);
      ...
      //     strncpy(housekeeping->request_buffer, buffer, strlen(buffer) + 1);
      //     pthread_exit(0);
      //     return NULL;
      // }
      ...
      printf("Requested http path (changed for opening): %s\n", requested_http_path_sliced);





      //----- Handles the case in which something with Spaces is being requested
      ...
      // Add ignoring for favicon.ico

      // make sure to handle for a file that DNE
      if (strncmp(html_path_decoded,"favicon.ico", 11) != 0){
        ...
        if ((fp=fopen(final_path,"rb")) == NULL) { // opening file
          ...
          // Maybe wait for more connections?
          // close socket in simple server
          // surround rest with else to avoid reading from file that's not there
          ...
          // create the HTTP response header with the data read in appended at the end of the response header.
          char http_response_header[600000];
          memset(http_response_header, 0, 600000);
          ...
          // Content-Type
          printf("http path decoded below: %s\n", html_path_decoded);
          ...
          */
          ...
          //strncat(http_response_header, dest, strlen(dest));
          //printf("http_response_header with data:\n%s", http_response_header);
          ...
          printf("length of http response header: %d\n", strlen(http_response_header));
  ...
  */
  void * server_func(void * client_socket) {
    ...
    int is_first_request = 1; // 0 if not first request, 1 if it is first request.
    // struct thread_params {
    //     int client_soc;
    //     char request_buffer[30000];
    //     int is_first_request;
    ...
    while(1){
      if (connection_flag == 0) {
        ...
        printf("~~~~~~~~~~~~~~~~~~~~OPENED NEW TCP CONNECTION~~~~~~~~~~~~~~~~~~~~\n");
        connection_flag = 1;
      }
      printf("client socket value: %d\n", new_socket);

      // TODO: (persistantserver.c) Keep track of every new client that connects.
      ...
      timeout.tv_sec = 0;
      timeout.tv_usec = 500000; // this is in microseconds. Reference: 1,000,000 microseconds = 1 second
      rv = select(new_socket + 1, &set, NULL, NULL, &timeout);
      if (rv == -1) {
        ...
        printf("Reading client request...\n");
	      int thread_cnt;
        pthread_t thread_list[MAX_NUMBER_OF_REQUESTS];
        printf("Spinning up threads again...\n");
        // Creating MAX_NUMBER_OF_REQUESTS number of threads

        struct thread_params housekeeping;
        //housekeeping = (struct thread_params*)malloc(sizeof(struct thread_params));
        housekeeping.client_soc = new_socket;
        housekeeping.is_first_request = is_first_request;


        for (thread_cnt = 0; thread_cnt < MAX_NUMBER_OF_REQUESTS; thread_cnt++) {
            pthread_create(&thread_list[thread_cnt], NULL, &read_requests, (void *) &housekeeping);
            //pthread_create(&thread_list[thread_cnt], NULL, &read_requests, (void *) new_socket);
        }

        printf("Created %d new threads\n", thread_cnt);

        printf("Joining threads again...\n");
        // Running threads
        for (thread_cnt = 0; thread_cnt < MAX_NUMBER_OF_REQUESTS; thread_cnt++) {
          pthread_join(thread_list[thread_cnt], NULL);
        }
        printf("now in parent thread\n");
        if (is_first_request == 0) {
          printf("housekeeping.buffer in main thread: %s\n", housekeeping.request_buffer);
        }
        is_first_request = 0;
        if (housekeeping.connection_flag == 0) {
          connection_flag = 0;
          close(new_socket);
          printf("No new request recieved from socket %d\n", new_socket);
          printf("Closed socket %d\n", new_socket);
    ...
    int main(int argc, char *argv[]) {
      ...
      // No need for HTTP response, just console print
      ...
      int client_cnt;
      pthread_t thread_list[QUEUE_LENGTH];
      int client_soc = -2;
      printf("Spinning up threads...\n");
      /* Creating QUEUE_LENGTH number of threads */
